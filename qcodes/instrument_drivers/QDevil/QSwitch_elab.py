...
class QSwitches():
    """
    Class to control multiple QSwitches as a single instrument
    Args: qswitch (list[QSwitch]): List of QSwitch objects
        default_names (ArrayLike): Array of default names for the relays. If None, the default names are set to 1 to 24*len(qswitch)
    """
    def __init__(self, qswitch: list[QSwitch], default_names=None):
    ...
    def _set_default_names(self, default_names = None) -> None:
        if default_names:
            lines = default_names
        else:
            lines = arange(1, relay_lines*len(self.qswitches))
        taps = range(1, relays_per_line)
        self._line_names = dict(zip(map(str, lines), lines))
        self._tap_names = dict(zip(map(str, taps), taps))
